...
public class PurchaseHistoryServiceImpl implements PurchaseHistoryService {
    ...
    public boolean record(PurchaseHistory ph) {
        logger.info("record() called for userId={} bookId={} quantity={} date={}",
                ph.getUserId(), ph.getBookId(), ph.getQuantity(), ph.getPurchaseDate());
        boolean result = historyDao.save(ph);
        if (result) {
            logger.info("PurchaseHistory saved successfully with id={}", ph.getId());
        } else {
            logger.warn("Failed to save PurchaseHistory for userId={} bookId={}",
                    ph.getUserId(), ph.getBookId());
        }
        return result;
    }
    ...
    public List<PurchaseHistory> getByUserId(Long userId) {
        logger.info("getByUserId() called for userId={}", userId);
