...
public class BasketDaoImpl implements BasketDao {
        ...
        try (Connection conn = getConnection()) {
            ...
            try (PreparedStatement psUpdate = conn.prepareStatement(updateSql)) {
                ...
                if (updated > 0) {
                    ...
                    conn.commit();
                    logger.info("Quantity updated successfully for userId={}, bookId={} (delta={})", userId, bookId, delta);
                    return true;
                }
            }
            ...
            conn.commit();
            logger.info("New basket item inserted for userId={}, bookId={}, quantity={}", userId, bookId, delta);
            return true;
    ...
    public boolean setQuantity(Long userId, Long bookId, int newQuantity) {
        ...
        try (Connection conn = getConnection();
            ...
            if (updated) {
                logger.info("Quantity set to {} for userId={}, bookId={}", newQuantity, userId, bookId);
            } else {
    ...
    public List<BasketItem> findByUserId(Long userId) {
        ...
        try (Connection conn = getConnection();
            ...
            logger.info("Fetched {} basket items for userId={}", result.size(), userId);
        } catch (SQLException e) {
    ...
    public boolean deleteItem(Long userId, Long bookId) {
        ...
        try (Connection conn = getConnection();
            ...
            if (deleted) {
                logger.info("Deleted basket item for userId={}, bookId={}", userId, bookId);
            } else {
    ...
    public boolean deleteAllByUserId(Long userId) {
        ...
        try (Connection conn = getConnection();
            ...
            boolean deleted = ps.executeUpdate() > 0;
            if (deleted) {
                logger.info("Deleted all basket items for userId={}", userId);
            } else {
