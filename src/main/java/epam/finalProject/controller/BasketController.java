...
public class BasketController {
    ...
    public BasketController(BasketService basketService,
        ...
        logger.info("BasketController initialized");
    }
    ...
    public String changeQuantity(@PathVariable Long bookId,
        ...
        logger.info("POST /basket/change/{}/{} - user='{}' changing quantity by {}", bookId, delta, username, delta);
        if (auth != null) {
    ...
    public String setQuantity(@PathVariable Long bookId,
                              ...
                              Authentication auth) {
        String username = (auth != null) ? auth.getName() : "anonymous";
        logger.info("POST /basket/set/{} - user='{}' setting quantity to {}", bookId, username, quantity);
        if (auth != null) {
            User user = userService.getByUsername(username);
            if (user != null) {
    ...
    public String removeItem(@PathVariable Long bookId,
                             Authentication auth) {
        String username = (auth != null) ? auth.getName() : "anonymous";
        logger.info("POST /basket/remove/{} - user='{}' removing item from basket", bookId, username);
        if (auth != null) {
            User user = userService.getByUsername(username);
            if (user != null) {
    ...
    public String viewBasket(Authentication auth, Model model) {
        String username = (auth != null) ? auth.getName() : "anonymous";
        logger.info("GET /basket - user='{}' viewing basket", username);
    ...
    public String confirmAll(Authentication auth, Model model) {
        String username = (auth != null) ? auth.getName() : "anonymous";
        logger.info("POST /basket/confirm - user='{}' confirming entire basket purchase", username);
        ...
        if (auth != null) {
            ...
            if (user != null) {
                ...
                if (success) {
                    logger.info("Basket purchase confirmed for userId={}", user.getId());
                } else {
        ...
        if (!success) {
            ...
            // Reuse viewBasket(...) to repopulate model and return basket view
