...
public class BasketServiceImpl implements BasketService {
    ...
    public boolean changeQuantity(Long userId, Long bookId, int delta) {
        logger.info("changeQuantity() called for userId={} bookId={} delta={}", userId, bookId, delta);

        // Retrieve current basket quantity for the book
        ...
        // Perform the DAO update/insert
        boolean result = basketDao.addOrUpdateQuantity(userId, bookId, delta);
        if (result) {
            logger.info("Basket quantity updated for userId={} bookId={} delta={}", userId, bookId, delta);
        } else {
    ...
    public boolean setQuantity(Long userId, Long bookId, int newQuantity) {
        logger.info("setQuantity() called for userId={} bookId={} newQuantity={}", userId, bookId, newQuantity);
        ...
        if (book.getQuantity() < newQuantity) {
            logger.warn("Insufficient stock for bookId={} requested newQuantity={} available={}",
                    bookId, newQuantity, book.getQuantity());
            return false;
        }

        boolean result = basketDao.setQuantity(userId, bookId, newQuantity);
        if (result) {
            logger.info("Basket quantity set to {} for userId={} bookId={}", newQuantity, userId, bookId);
        } else {
    ...
    public boolean removeItem(Long userId, Long bookId) {
        logger.info("removeItem() called for userId={} bookId={}", userId, bookId);
        boolean result = basketDao.deleteItem(userId, bookId);
        if (result) {
            logger.info("Removed basket item for userId={} bookId={}", userId, bookId);
        } else {
    ...
    public boolean clearBasket(Long userId) {
        logger.info("clearBasket() called for userId={}", userId);
        boolean result = basketDao.deleteAllByUserId(userId);
        if (result) {
            logger.info("Cleared all basket items for userId={}", userId);
        } else {
    ...
    public List<BasketItem> getBasketItems(Long userId) {
        logger.info("getBasketItems() called for userId={}", userId);
        List<BasketItem> items = basketDao.findByUserId(userId);
    ...
    public boolean confirmAll(Long userId) {
        logger.info("confirmAll() called for userId={}", userId);

        // Fetch current basket items
        List<BasketItem> items = basketDao.findByUserId(userId);
        ...
        for (BasketItem it : items) {
            ...
            if (book.getQuantity() < it.getQuantity()) {
                logger.warn("Insufficient stock for bookId={} requested={} available={}",
                        it.getBookId(), it.getQuantity(), book.getQuantity());
                return false;
            }
        }

        // Process each item: decrement stock, record history, remove from basket
        for (BasketItem it : items) {
            ...
            if (!savedHistory) {
                logger.error("Failed to save purchase history for userId={} bookId={} quantity={}",
                        userId, it.getBookId(), it.getQuantity());
                return false;
            }
            ...
            if (!removed) {
                logger.error("Failed to remove basket item during confirmation for userId={} bookId={}",
                        userId, it.getBookId());
                return false;
            }
        ...
        logger.info("All basket items confirmed successfully for userId={}", userId);
        return true;
    }
}
