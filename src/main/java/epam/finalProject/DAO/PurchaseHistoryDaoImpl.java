...
public class PurchaseHistoryDaoImpl implements PurchaseHistoryDao {
    ...
    public boolean save(PurchaseHistory record) {
        ...
        logger.info("save(PurchaseHistory) called for userId={}, bookId={}, quantity={}",
                record.getUserId(), record.getBookId(), record.getQuantity());
        ...
        try (Connection conn = getConnection();
            ...
            logger.debug("Executing INSERT: {} with values userId={}, bookId={}, quantity={}, purchaseDate={}",
                    sql, record.getUserId(), record.getBookId(), record.getQuantity(), record.getPurchaseDate());
            ...
            try (ResultSet keys = ps.getGeneratedKeys()) {
                if (keys.next()) {
                    ...
                    logger.info("PurchaseHistory saved with generated ID={}", generatedId);
                }
            }
            return true;

        } catch (SQLException e) {
            logger.error("Database error in save(PurchaseHistory): userId={}, bookId={}, quantity={}",
                    record.getUserId(), record.getBookId(), record.getQuantity(), e);
            return false;
        }
    }
    ...
    public List<PurchaseHistory> findByUserId(Long userId) {
        ...
        try (Connection conn = getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                logger.info("Fetched {} purchase history records for userId={}", result.size(), userId);
            }
