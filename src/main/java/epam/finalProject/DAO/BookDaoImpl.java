...
import epam.finalProject.exception.DaoException;
...
public class BookDaoImpl implements BookDao {
    ...
    public BookDaoImpl(DataSource ds) {
        this.ds = ds;
        logger.info("BookDaoImpl initialized with provided DataSource");
    }
    ...
    public BookDaoImpl() {
        this.ds = null;
        logger.info("BookDaoImpl initialized using default ConnectionPool");
    }
    ...
    public boolean deleteBook(Book book) {
        logger.info("deleteBook() called for book id={}", book.getId());
        try (Connection conn = getConnection();
            ...
            if (deleted) {
                logger.info("Book deleted successfully: id={}", book.getId());
            } else {
    ...
    public boolean changeBook(Book book) {
        logger.info("changeBook() called for book id={}", book.getId());
        try (Connection conn = getConnection();
            ...
            logger.debug("Executing UPDATE: {} with values title={}, authorId={}, year={}, description={}, quantity={}, id={}",
                    UPDATE_BOOK_SQL,
                    book.getTitle(), book.getAuthorId(), book.getYear(),
                    book.getDescription(), book.getQuantity(), book.getId());
    ...
    public List<Book> findAll(Pageable pageable) {
        ...
        try (Connection conn = getConnection();
            ...
            // Подстановка параметров пагинации
            ...
            throw new DaoException("Ошибка при получении списка книг с пагинацией", e);
        }
    }
    ...
    public Book findById(Long id) {
        logger.info("findById() called for book id={}", id);
        try (Connection conn = getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    logger.info("Book found: id={} title='{}'", book.getId(), book.getTitle());
                    return book;
                } else {
            ...
            throw new DaoException("Ошибка при получении книги по id=" + id, e);
        }
    }
    ...
    public boolean saveBookWithAuthor(Book book, Author author) {
        logger.info("saveBookWithAuthor() called for book title='{}' author='{}'", book.getTitle(), author.getName());
        try (Connection conn = getConnection()) {
            ...
            logger.info("Book saved successfully with id={} and authorId={}", bookId, authorId);
            return true;
        } catch (SQLException e) {
    ...
    public boolean decrementQuantity(Long bookId, int amount) {
        logger.info("decrementQuantity() called for bookId={} amount={}", bookId, amount);
        ...
        try (Connection conn = getConnection();
            ...
            boolean updated = ps.executeUpdate() > 0;
            if (updated) {
                logger.info("Quantity decremented by {} for bookId={}", amount, bookId);
            } else {
        ...
        try (PreparedStatement ps = conn.prepareStatement(FIND_AUTHOR_SQL)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    logger.info("Author found with id={} for name='{}'", existingId, author.getName());
                    return existingId;
                }
            }
        }
        ...
        try (PreparedStatement ps = conn.prepareStatement(INSERT_AUTHOR_SQL, Statement.RETURN_GENERATED_KEYS)) {
            ...
            try (ResultSet rs = ps.getGeneratedKeys()) {
                if (rs.next()) {
                    ...
                    logger.info("New author created with id={} name='{}'", newId, author.getName());
                    return newId;
                }
            }
        }
        throw new SQLException("Unable to get or create author ID for name='" + author.getName() + "'");
    }
    ...
    private long insertBook(Connection conn, Book book, long authorId) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(INSERT_BOOK_SQL, Statement.RETURN_GENERATED_KEYS)) {
            ...
            ps.setString(4, book.getDescription());
            ps.setInt(5, book.getQuantity());
            logger.debug("Executing INSERT: {} with title='{}', year={}, authorId={}, description='{}', quantity={}",
                    INSERT_BOOK_SQL,
                    book.getTitle(), book.getYear(), authorId, book.getDescription(), book.getQuantity());
            ps.executeUpdate();
            try (ResultSet rs = ps.getGeneratedKeys()) {
                if (rs.next()) {
                    long newBookId = rs.getLong(1);
                    logger.info("New book inserted with id={} title='{}'", newBookId, book.getTitle());
                    return newBookId;
                }
            }
        }
        throw new SQLException("Unable to get generated book ID for title='" + book.getTitle() + "'");
    }
    ...
    private void insertBookGenres(Connection conn, long bookId, List<Long> genreIds) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(INSERT_BOOK_GENRE_SQL)) {
            ...
            ps.executeBatch();
            logger.info("Inserted {} genre associations for bookId={}", genreIds.size(), bookId);
        }
    }
    ...
    private List<Genre> loadGenres(Connection conn, long bookId) throws SQLException {
        ...
        logger.info("Loaded {} genres for bookId={}", list.size(), bookId);
        return list;
    }
